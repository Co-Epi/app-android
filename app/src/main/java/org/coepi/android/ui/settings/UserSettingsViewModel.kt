...
class UserSettingsViewModel(
    ...
    val showWebSubject = create<Uri>()
    val showWeb = showWebSubject.toLiveData()
//    val showWeb: MutableLiveData<Event<Uri>> = MutableLiveData()
    ...
    fun onClick(item: Text, activity: Activity) {
        when (item.id) {
            REPORT_PROBLEM -> email.open(activity, "TODO@TODO.TODO", "TODO")
            PRIVACY_STATEMENT ->
                showWebSubject.onNext(parse("https://www.coepi.org/privacy/"))
            else -> {}
        }
    }
